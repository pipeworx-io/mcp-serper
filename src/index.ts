...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Serper');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'search',
    description: 'Google web search via Serper: returns organic results (title, link, snippet, position), an answer box if present, and a knowledge graph summary. Accepts query, result count (max 20), and optional country code (gl) and location for geo-targeted results.',
    inputSchema: {
  ...
  {
    name: 'news',
    description: 'Google News search via Serper: returns recent news articles (title, link, snippet, source, date) matching the query. Accepts query, result count (max 20), and optional country code (gl).',
    inputSchema: {
      type: 'object' as const,
      properties: {
        query: {
  ...
  {
    name: 'places',
    description: 'Google Maps local business search via Serper: returns matching places with title, address, rating, review count, category, phone number, and website. Accepts query plus optional country code (gl) and location string for geo-targeting.',
    inputSchema: {
      type: 'object' as const,
      properties: {
        query: {
  ...
  async function news(a: Record<string, unknown>) {
    ...
    return {
      news: (d.news || []).map((n) => ({
        ...
        source: n.source,
        date: n.date,
        published_at: normalizeDate(n.date),
      })),
    };
  }
...
function normalizeDate(raw: string | undefined): string | undefined {
  ...
  if (/\b(hour|minute|min|second)s? ago\b/i.test(raw)) return today;
  const dayMatch = raw.match(/^(\d+)\s+days?\s+ago$/i);
  if (dayMatch) { const d = new Date(now); d.setDate(d.getDate() - Number(dayMatch[1])); return d.toISOString().slice(0, 10); }
  const weekMatch = raw.match(/^(\d+)\s+weeks?\s+ago$/i);
  if (weekMatch) { const d = new Date(now); d.setDate(d.getDate() - Number(weekMatch[1]) * 7); return d.toISOString().slice(0, 10); }
  const monthMatch = raw.match(/^(\d+)\s+months?\s+ago$/i);
  if (monthMatch) { const d = new Date(now); d.setMonth(d.getMonth() - Number(monthMatch[1])); return d.toISOString().slice(0, 10); }
  const parsed = new Date(raw);
  return Number.isNaN(parsed.getTime()) ? undefined : parsed.toISOString().slice(0, 10);
}

// Serper is genuinely funded — we buy credits. VENDOR_QUOTAS pins the price at
// $0.001/call (sourced to serper.dev, and a unit test asserts the funded daily
// budget stays consistent with it). House formula: ceil(0.001 * 1.5 * 15000).
export default { tools, callTool, cost: { base: 23 }, provider: 'serper' } satisfies McpToolExport;
